...
function pointZero(arr: [number, number][]){
    ...
    let result = array[0];
    let distance = Math.sqrt(result[0] ** 2 + result[1] ** 2);

    for (let i = 1; i < array.length; i++) {
        const item = array[i];
        const itemDisatnce = Math.sqrt(item[0] ** 2 + item[1] ** 2);
        if (itemDisatnce < distance) {
            result = item;
            distance = itemDisatnce;
        }
    }
...
const A : [number,number][] = [[1,2],[-1,3],[0,0],[1,1]];
